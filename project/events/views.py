...
class DeleteEvents(GenericAPIView):
    ...
    def post(self, request:Request) -> Response:
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        return Response(bulk_delete_events(serializer=serializer,queryset=self.queryset,user=request.user),status=HTTP_200_OK)
...
class BulkAcceptOrDeclineRequestToParticipation(GenericAPIView):
    ...
    def post(self,request:Request) -> Response:
        serializer = self.serializer_class(data=request.data)
        serializer.is_valid(raise_exception=True)
        return Response(bulk_accpet_or_decline(serializer=serializer,queryset=self.queryset,user=request.user),status=HTTP_200_OK)