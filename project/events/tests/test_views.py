...
class TestEventsViews(SetUpEventsViews):
    ...
    def test_event_fan_join_to_event(self) -> None:
        self.create_events(1)
        self.register_second_user()
        fan_join_to_event = self.client.post(reverse('spectator-join-to-event'), {'event_id': Event.objects.first().id})
        event_join = self.client.post(reverse('join-to-event'), {'event_id': Event.objects.first().id})
    ...
    def test_leave_from_event(self) -> None:
        ...
        event_join = self.client.post(reverse('join-to-event'), {'event_id': Event.objects.first().id})
        self.assertEqual(Event.objects.first().count_current_users, 1)
        event_leave = self.client.post(reverse('leave-from-event'), {'event_id': Event.objects.first().id})
    ...
    def test_invite_user_to_event(self) -> None:
        ...
        self.auth()
        self.assertEqual(Notification.objects.count(), 0)
        response = self.client.post(reverse('invite-to-event'), {
            'user_id': User.objects.get(email = self.user_reg_data_2['email']).id,
            'event_id': Event.objects.first().id
        })
        self.assertEqual(Notification.objects.count(), 1)
        self.assertEqual(Notification.objects.first().user.email, self.user_reg_data_2['email'])
        self.assertEqual(response.status_code, HTTP_200_OK)
    ...
    def register_second_user(self) -> NoneType:
        ...
        return self.client.force_authenticate(User.objects.get(email = self.user_reg_data_2['email']))
