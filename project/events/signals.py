...
def send_to_scedular_after_new_user_join_to_event(sender: User, instance: User, **kwargs: Any) -> None:
    ...
    if action == 'post_add':
        send_to_scedular(user = instance, data = {'event': {'data': str(instance.current_rooms.through.objects.last().event.__dict__), 
            'type': 'new_event'}})
        print(instance.current_rooms.through.objects.last().event.count_current_users)
