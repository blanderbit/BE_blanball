...
class TestAuthenticationViews(SetUpAauthenticationViews):
    ...
    def test_user_register_with_bad_birthday(self) -> None:
        response = self.client.post(reverse('register'),self.user_register_bad_birthday_date)
    ...
    def test_check_change_phone(self) -> None:
        ...
        self.assertEqual(User.objects.first().phone, self.request_change_phone_data['phone'])
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_change_phone_with_same_data(self) -> None:
        self.auth()
        response = self.client.post(reverse('request-change-phone'),{'phone': self.user_register_data['phone']})
    ...
    def test_change_email(self) -> None:
        ...
        self.user_register_data['password']})
        self.assertEqual(login_user_with_new_email.status_code, status.HTTP_200_OK)
        self.assertEqual(login_user_with_old_email.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(User.objects.first().email, self.request_change_email_data['email'])
        self.assertEqual(response.status_code, status.HTTP_200_OK)

    def test_change_email_with_same_data(self) -> None:
        self.auth()
        response = self.client.post(reverse('request-change-phone'),{'email': self.user_register_data['email']})
        self.assertEqual(Code.objects.count(),0)
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_reset_password(self) -> None:
        ...
        self.client.post(reverse('register'),self.user_register_data)
        with freeze_time('2022-9-28-14:00'):
            request_reset = self.client.post(reverse('request-reset-password'),{'email':self.user_register_data['email']})
        with freeze_time('2022-9-28-14:01'):
            response = self.client.post(reverse('complete-reset-password'),{'verify_code':Code.objects.first().verify_code,
            'new_password':new_pass})
        login_user_with_old_pass = self.client.post(reverse('login'),{'email':self.user_register_data['email'],'password':
        self.user_register_data['password']})
        login_user_with_new_pass = self.client.post(reverse('login'),{'email':self.user_register_data['email'],'password':
        new_pass})
        self.assertEqual(login_user_with_new_pass.status_code, status.HTTP_200_OK)
        self.assertEqual(login_user_with_old_pass.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_change_password(self) -> None:
        self.auth()
        self.client.post(reverse('request-change-password'),self.request_change_password_data)
        response = self.client.post(reverse('check-code'),{'verify_code':Code.objects.first().verify_code})
        self.client.force_authenticate(None)
        login_user_with_old_pass = self.client.post(reverse('login'),{'email':self.user_register_data['email'],'password':
        self.user_register_data['password']})
    ...
    def test_get_user_profile(self) -> None:
        self.auth()
        response = self.client.get(reverse('user-profile',kwargs={'pk': User.objects.first().id}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_delete_account(self) -> None:
        ...
        response = self.client.post(reverse('check-code'),{'verify_code':Code.objects.first().verify_code})
        self.assertEqual(User.objects.count(),0)
        self.assertEqual(request_delete.status_code, status.HTTP_200_OK)
        self.assertEqual(response.status_code, status.HTTP_200_OK)




    # def test_email_verify_with_verified_account(self):
    #     self.auth()
    #     request_verify = self.client.get(reverse('request-email-verify'))
    #     response = self.client.post(reverse('check-code'),{'verify_code':Code.objects.first().verify_code})
    #     request_verify_2 = self.client.get(reverse('request-email-verify'))
    #     self.assertEqual(response.status_code, status.HTTP_200_OK)
    #     self.assertEqual(request_verify_2.status_code, status.HTTP_200_OK)
